 ...
 * owning workspace and to the profile + backend ref where the bytes live. `form_version_id` and `submission_id`
 * are optional tags (no FKs) — uploads can exist before a feature claims them.
 */
export const files = sobaSchema.table(
  ...
  (table) => ({
    ...
    formIdx: index('file_form_id_idx').on(table.formId),
