...
- `SOBA (Backend + Outbox + Temporal + Frontend)`: start all services including frontend
- `SOBA (Backend + Outbox + Temporal)`: backend stack only — run `soba-fe` in a terminal for frontend with personal memory settings (see `.devcontainer/README.md`)