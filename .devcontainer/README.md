...
Templates use their own storage profile (`templates`), separate from the `default` profile the files feature uses; each can run a different backend at the same time (memory, local disk, or S3). To
