 ...
 * replace it later.
 */
function transformChefs1Types(schema) {
...
function repairComponentWidget(c) {
    if ('widget' in c && !isPlainObject(c.widget))
        delete c.widget;
...
function repairWidgetsInRow(row) {
    for (const cell of row) {
        if (isPlainObject(cell))
            repairWidgets(cell.components);
    }
}
...
function repairNestedComponents(c) {
    repairWidgets(c.components);
    if (Array.isArray(c.columns)) {
        for (const col of c.columns) {
            if (isPlainObject(col))
                repairWidgets(col.components);
        }
    }
    if (Array.isArray(c.rows)) {
        for (const row of c.rows) {
            if (Array.isArray(row))
                repairWidgetsInRow(row);
        }
    }
}
/**
