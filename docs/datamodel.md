...
- **Core:** Identity providers and app users; `user_identity` links users to IdPs (subject, provider). Workspaces have an optional owner (`app_user`); `workspace_membership` links users to workspaces (role, status, invite/accept). `workspace_group` and `workspace_group_membership` model groups within a workspace and which members belong to which groups.
...
|-------|-------------|-------------|
| identity_provider | core.ts | IdP config (code, name, active). |
| app_user | core.ts | Users; display label, profile JSON, status. |
| user_identity | core.ts | Links users to IdPs (subject, external id, idp attributes). |
| workspace | core.ts | Workspaces (kind, name, slug, status, owner). |
| workspace_membership | core.ts | User–workspace link (role, status, source, invite/accept). |
...
| form | forms.ts | Form definition (workspace, form_engine_code, slug, name, status; soft delete). |
| form_version | forms.ts | Version of a form (version_no, state, engine schema/sync, published at/by). `engine_schema_ref` = form engine’s ID for the schema record (e.g. Form.io MongoDB _id). |
| form_version_revision | forms.ts | Revision history for a form version. |
| submission | forms.ts | Submission (form, version, submitter, workflow state, engine ref/sync; soft delete). `engine_submission_ref` = form engine’s ID for the submission record (e.g. Form.io MongoDB _id). |
| submission_revision | forms.ts | Revision history for a submission. |
| integration_outbox | integration.ts | Outbox for async integration events (topic, aggregate, payload, status, retries). |
| personal_workspace_settings | plugins.personal.ts | Per-workspace user preferences (e.g. default landing path). |
| personal_invite | plugins.personal.ts | Email invites (invitee, token hash, status, expiry, accept). |
| personal_audit | plugins.personal.ts | Audit log (actor, event type, payload, occurred at). |
| enterprise_workspace_binding | plugins.enterprise.ts | Workspace ↔ external provider workspace. |
| enterprise_membership_binding | plugins.enterprise.ts | Membership ↔ external provider membership/identity. |
| enterprise_group_binding | plugins.enterprise.ts | Workspace group ↔ external provider group. |
